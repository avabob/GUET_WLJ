...
class Gift:
	...
	def ConstraintsBySbox(self, variable1, variable2):
		...
		for k in range(0,16):   #k的值取决于中间状态的长度以及s盒的结构。若s盒为4*4 = 16，中间状态为64位则需要
			for coff in Gift.S_T: #coff是s盒的多项式系数和常数项，前八位为系数，最后一位为常数项
				...
				for v in range(0,4):
					temp.append(str(coff[v + 4]) + " " + variable2[(k * 4) + 3 - v]) #v + 4的值为4，5，6，7 ; (k * 4) + 3 - v的值为3，2，1，0
				temp1 = " + ".join(temp)
				temp1 = temp1.replace("+ -", "- ")
	...
	@staticmethod
	def LinearLaryer(variable):  #对应文章https://zhuanlan.zhihu.com/p/461549805中的p-Layer
		"""
	...
	def Constraint(self):
		...
		fileobj.close()
		variablein = Gift.CreateVariables(0)
		variableout = Gift.CreateVariables(1)
		if self.round == 1:
			...
			for i in range(1, self.round):
				variablein = Gift.LinearLaryer(variableout)
				variableout= Gift.CreateVariables(i + 1)
				self.ConstraintsBySbox(variablein, variableout)
				#omit the last linear layer

	def VariableBinary(self):  #写入二进制变量
		"""
	...
	def Init(self):
		...
		生成由初始分割属性引入的初始约束条件。
		"""
		input_state = Gift.CreateVariables(0)
	...
	def WriteObjective(self, obj):
		"""
		Write the objective value into result_file_name.
		"""
