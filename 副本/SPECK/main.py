from SPECK import Speck  # 从 mibs 模块导入 Mibs 类

# 主程序入口
if __name__ == "__main__":  # 判断脚本是否作为主程序运行
    ################输入分组长度
    print("可选分组长度为：[32, 48, 64, 96, 128]")
    blocksize = int(input("请输入目标密码的分组长度（32表示SPECK-32）:"))
    while blocksize not in [32, 48, 64, 96, 128]:
        print("无效的分组长度！")
        blocksize = int(input("请重新输入分组长度："))
    ...
    Speck = Speck(rounds, blocksize)  # 创建一个Mibs对象，初始化时传入目标回合数
    ...
    Speck.set_brute_force_flag(brute_force_flag)  # 设置Mibs对象的暴力破解标志
    Speck.set_constant_bits()
