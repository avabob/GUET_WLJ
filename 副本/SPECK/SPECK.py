...
x_0_0_0 表示第 0 轮，第 0 个字节的第 0 个比特（msb，即最重要的比特）。
x_0_0_3 表示第 0 轮，第 0 个字节的第 3 个比特（lsb，即最不重要的比特）。
...
class Speck:
    def __init__(self, round ,blocksize):
        ...
        self.model_file_name = "./model/Simon_%d.lp" % self.round
        # 设置结果文件的文件名，存放路径为 "result" 文件夹
        self.result_file_name = "./result/Simon_round%d_result.txt" % self.round
